...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"node_modules/pokemontcgsdk/es/configure.js":[function(require,module,exports) {
"use strict";
...
},{"./lib/axios":"node_modules/axios/lib/axios.js"}],"node_modules/es-errors/index.js":[function(require,module,exports) {
'use strict';
...
},{"./stringify":"node_modules/qs/lib/stringify.js","./parse":"node_modules/qs/lib/parse.js","./formats":"node_modules/qs/lib/formats.js"}],"node_modules/pokemontcgsdk/es/queryBuilder.js":[function(require,module,exports) {
"use strict";
...
},{"axios":"node_modules/axios/index.js","qs":"node_modules/qs/lib/index.js","./configure":"node_modules/pokemontcgsdk/es/configure.js"}],"node_modules/pokemontcgsdk/es/index.js":[function(require,module,exports) {
"use strict";
...
var configure = function configure(_ref) {
  ...
  _configure.default.apiKey = apiKey;
};
var _default = exports.default = {
  configure: configure,
  card: (0, _queryBuilder.default)('cards'),
  set: (0, _queryBuilder.default)('sets'),
  type: (0, _queryBuilder.default)('types'),
  subtype: (0, _queryBuilder.default)('subtypes'),
  rarity: (0, _queryBuilder.default)('rarities'),
  supertype: (0, _queryBuilder.default)('supertypes')
};
...
function fetchSetData() {
  _pokemontcgsdk.default.set.all().then(function (response) {
    sets = response.map(function (set) {
      var cardIds = Array.from({
        length: set.printedTotal
      }, function (_, i) {
        return i + 1;
      }).map(function (num) {
        return "".concat(set.id, "-").concat(num);
      });
      var logo = set.images.logo;

      // If the set name is "Twilight Masquerade", replace the logo URL
      if (set.name === 'Twilight Masquerade') {
        logo = _sv6Logo.default;
      }
      return {
        id: set.id,
        name: set.name,
    ...
    var series = _toConsumableArray(new Set(sets.map(function (set) {
      return set.series;
    })));

    // Get the series dropdown
    var seriesDropdown = document.getElementById('series-options');
    ...
    seriesDropdown.addEventListener('change', function (event) {
      // If "All Series" is selected, display all sets
      if (event.target.value === 'all') {
        displaySets(sets);
      } else {
        // Otherwise, filter the sets by the selected series
        var filteredSets = sets.filter(function (set) {
          return set.series === event.target.value;
        });
    ...
    searchBar.addEventListener('input', function (event) {
      ...
      var selectedSeries = document.getElementById('series-options').value;

      // If a series is selected and it's not "All Series", filter the sets by the selected series
      var setsToSearch = sets;
      if (selectedSeries && selectedSeries !== 'all') {
        setsToSearch = sets.filter(function (set) {
          return set.series === selectedSeries;
        });
      }

      // Search the setsToSearch by the search bar input
      var filteredSets = setsToSearch.filter(function (set) {
        return set.name.toLowerCase().includes(event.target.value.toLowerCase());
      });
      displaySets(filteredSets);
    });
    displaySets(sets);
...
function sortSets(sets, sortState) {
  var sortedSets = _toConsumableArray(sets); // Create a copy of the sets array
